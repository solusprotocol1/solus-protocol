...
function showRoleSelector() {
    ...
    modal.id = 'roleModal';
    modal.style.cssText = 'position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.3);z-index:10000;display:flex;align-items:center;justify-content:center;animation:fadeIn 0.3s ease';
    ...
    contentDiv.innerHTML = '<h3 style="color:#1d1d1f;margin:0 0 4px"><i class="fas fa-user-cog" style="color:#00aaff;margin-right:8px"></i>Configure Your Role</h3>'
        + '<p style="color:#666;font-size:0.85rem;margin-bottom:20px">Select your role to see relevant tools. You can customize visible tools and your displayed title.</p>'
        + '<div style="margin-bottom:16px"><label style="color:#666;font-size:0.8rem;font-weight:600">Your Display Title</label><input id="roleTitle" value="'+(_currentTitle||'')+'" style="background:#fff;color:#1d1d1f;border:1px solid rgba(255,255,255,0.15);border-radius:3px;padding:10px;width:100%;margin-top:4px;box-sizing:border-box" placeholder="e.g., ILS Analyst, Logistics Specialist, Contract Manager"></div>';
    ...
    for (var i = 0; i < roleKeys.length; i++) {
        (function(key) {
            ...
            card.innerHTML = '<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;pointer-events:none"><i class="fas '+r.icon+'" style="color:#00aaff;font-size:1.1rem"></i><strong style="color:#fff;font-size:0.9rem">'+r.label+'</strong></div>'
                + '<div style="color:#666;font-size:0.78rem;pointer-events:none">'+r.desc+'</div>'
    ...
    for (var t = 0; t < _allHubTabs.length; t++) {
        ...
        detailsHtml += '<label style="display:flex;align-items:center;gap:8px;font-size:0.82rem;color:#666;cursor:pointer;padding:6px 10px;border-radius:3px;background:rgba(255,255,255,0.02);border:1px solid rgba(0,0,0,0.06);transition:all 0.2s"><input type="checkbox" data-tab="'+tab+'" '+(vis?'checked':'')+' style="accent-color:#00aaff;width:16px;height:16px;flex-shrink:0"> <span>' + (_allHubLabels[tab]||tab) + '</span></label>';
    }
    ...
    cancelBtn.style.cssText = 'background:rgba(255,255,255,0.06);color:#666;border:1px solid rgba(0,0,0,0.1);border-radius:3px;padding:8px 20px;cursor:pointer';
