...
function toolActionAnchor() {
    // Navigate to Anchor tab with context from current tool
    var toolName = '';
    if (_currentILSTool) {
        var panel = document.getElementById(_currentILSTool);
        if (panel) {
    ...
    setTimeout(function() {
        ...
        // Pre-fill record input with context
...
function toolActionItem() {
    // Create an action item from current tool context
    ...
    // Add to s4ActionItems array if it exists
    if (typeof s4ActionItems !== 'undefined' && Array.isArray(s4ActionItems)) {
...
// ═══ Quick Tour (lightweight guided walkthrough) ═══
var _tourSteps = [
