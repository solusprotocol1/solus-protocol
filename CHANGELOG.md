...
- **DoW consent banner** — Recolored from orange (#ff9500) to brand blue (#00aaff)
...
- **Government tier removed** — Enterprise tier now covers DoW, federal agencies, and FedRAMP pathway.
...
- **Renamed "DoW / DoN Database Import" → "Defense Database Import"** — Reflects full coverage of the U.S. Navy, not just Navy/DoW.
...
- **DoW Database Import Adapters** — 13 DoW/DoN system adapters added to `S4SDK`: NSERC/SE IDE, MERLIN, NAVAIR AMS PMT, COMPASS, CDMD-OA, NDE, MBPS, PEO MLB, CSPT, GCSS, DPAS, DLA FLIS/WebFLIS, NAVSUP OneTouch. Each adapter includes system metadata, supported formats (CSV/XML/JSON/fixed-width), and mapped record types. Import methods: `import_csv()`, `import_xml()`, `import_json()`, `import_and_anchor()`, `list_dod_systems()`.
- **DoW Import Tool in Demo App** — New 19th ILS tool "DoW Import" in `demo-app/index.html`. File upload (drag-and-drop) and paste support for CSV/XML/JSON. Source system selector (13 systems), format selector, auto-detect record types. Demo import generator with realistic sample data for each system. Import, hash, anchor, and export workflows.
- **Metrics: Data Source Tracking** — `/api/metrics` now tracks `records_by_source` showing which DoW system each record was imported from (NSERC, CDMD-OA, MERLIN, etc.).

### Added — Documentation
- **DoW Database Integration Sections** — Added to TECHNICAL_SPECS.md, WHITEPAPER.md, and S4_SYSTEMS_EXECUTIVE_PROPOSAL.md. Includes 13-system compatibility table, import workflow (Export → Upload → Parse → Hash → Anchor → Integrate), SDK code examples.
...
- **SDK Status Command Updated** — Now shows v3.9.7, 500+ platforms, 13 DoW import systems, 28 REST API endpoints, and 25+ platform pages.
...
- **DoDI Directive Name Fix** — Fixed "DoDI 4245.14" back to "DoDI 4245.14" in PRODUCTION_READINESS.md (official directive names keep DoW prefix). Fixed "DoW 5200.01" back to "DoW 5200.01" in BAA_TEMPLATE.md.
...
- **CHANGELOG.md Terminology** — Fixed remaining "DoW" → "DoW" in changelog entries where not referring to official directive names.
...
- **DoW Terminology Standardized** — Updated all general "DoW" / "Department of War" references to "DoW" / "Department of War" across 10+ documents (preserving official directive names like DoD 5000.73, DoDI 4245.15).
